...
**[EN PANTALLA: capa `mapas_bolivia:dptos_bol` con la pestaña Cacheado de Teselas ya abierta]** Así se ve en la práctica la primera vez que activas la caché de una capa, sin tocar nada todavía: las dos casillas de General settings marcadas, BlobStore en *(*) BlobStore por defecto*, Meta tiling en `4 × 4`, Gutter en `0`, y en Cache image formats ya vienen marcados `image/jpeg` e `image/png` —los dos formatos "de siempre" que mencionamos arriba—. Los dos campos de expiración quedan en `0` —heredan de la fuente—, y Skip caching on dimension warnings sin marcar, porque esta capa no tiene dimensiones TIME ni ELEVATION. En Parameter Filters, el filtro STYLES ya aparece con *Valores de capa por defecto* y *TODOS LOS ESTILOS* marcado. Y en Cached Grid Sets, sin que hayas agregado nada, ya están los dos gridsets que acabamos de explicar: `EPSG:4326` con Published `0 / Max` y Cached `Min / Max` —sin restricción—, y `EPSG:900913` con Published `Min / 20` y Cached `0 / 4` —GeoServer lo deja con un rango bajo por defecto—, ambos con Subset bounds en *Dynamic*. Es la plantilla estándar que trae GeoServer 3 apenas marcas "Create a Cached Layer": lo único que decides tú es qué formatos, qué estilos y qué rango de zoom de verdad quieres poblar con Seed.

Un matiz importante para no buscar donde no es: los botones **Seed, Truncate y Empty** —pre-generar, recortar y vaciar la caché— **no viven en esta pestaña**. Están en el menú global *Cacheado de Teselas* de la barra superior de GeoServer (el mismo nombre que la pestaña de la capa, pero es una sección distinta), en *Tile Layers → Seed/Truncate* de cada capa. Y ahí es donde ocurre la magia previa a cualquier demo en vivo: **Seed** pre-genera las teselas de una zona y un rango de zoom antes de que nadie las pida, así el primer clic de tu audiencia carga instantáneo en vez de tardar diez segundos delante de todos.
